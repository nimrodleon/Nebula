...
import { ProductStockService } from "app/inventory/services";
...
export class ProductListComponent implements OnInit {
  ...
  public get hasPermission(): boolean {
    return this.userDataService.hasPermission();
  }
        ...
        if (result.isConfirmed) {
          this.productService.delete(id)
            .subscribe(result => {
              ...
              this.router.navigate(["/", this.companyId, "products"]).then(() => console.log(result.id));
            });
        }
      });
    }
