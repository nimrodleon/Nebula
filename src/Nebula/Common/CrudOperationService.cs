...
public interface ICrudOperationService<T> where T : class, IGenericModel
{
    ...
    Task<T> InsertOneAsync(T obj);
    Task InsertManyAsync(List<T>? objList);
    Task<T> ReplaceOneAsync(string id, T obj);
    Task DeleteOneAsync(string id);
    Task DeleteOneAsync(string companyId, string id);
}

public class CrudOperationService<T>(MongoDatabaseService mongoDatabase) : ICrudOperationService<T>
    where T : class, IGenericModel
{
    protected readonly IMongoCollection<T> _collection = mongoDatabase.GetDatabase().GetCollection<T>(typeof(T).Name);
    ...
    public virtual async Task<List<T>> GetFilteredAsync(string companyId, string[] fieldNames, string query = "",
        int limit = 12)
    {
    ...
    public virtual async Task<T> InsertOneAsync(T obj)
    {
        obj.Id = string.Empty;
    ...
    public virtual async Task<T> ReplaceOneAsync(string id, T obj)
    {
    ...
    public virtual async Task DeleteOneAsync(string companyId, string id)
    {
        var filter = Builders<T>.Filter.And(
            Builders<T>.Filter.Eq(x => x.Id, id),
            Builders<T>.Filter.Eq("CompanyId", companyId)
