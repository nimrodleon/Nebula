<?php

namespace App\Form;

use App\Classes\InvoiceHub;
use Symfony\Component\Form\AbstractType;
use Symfony\Component\Form\Extension\Core\Type\CollectionType;
use Symfony\Component\Form\Extension\Core\Type\TextType;
use Symfony\Component\Form\FormBuilderInterface;
use Symfony\Component\OptionsResolver\OptionsResolver;

class InvoiceType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ->add('ruc', TextType::class)
            ->add('tipoOperacion', TextType::class)
            ->add('tipoDoc', TextType::class)
            ->add('serie', TextType::class)
            ->add('correlativo', TextType::class)
            ->add('fechaEmision', TextType::class)
            ->add('formaPago', FormaPagoType::class)
            ->add('fecVencimiento', TextType::class)
            ->add('cuotas', CollectionType::class, [
                'entry_type' => CuotaDtoType::class,
                'allow_add' => true,
                'by_reference' => false
            ])->add('tipoMoneda', TextType::class)
            ->add('client', ClientType::class)
            ->add('details', CollectionType::class, [
                'entry_type' => SaleDetailType::class,
                'allow_add' => true,
                'by_reference' => false
            ]);
    }

    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'data_class' => InvoiceHub::class
        ]);
    }
}
